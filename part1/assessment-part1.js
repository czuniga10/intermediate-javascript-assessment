...
String.prototype.grammarPolice = function() {
   // make lower cases and split each word at the space
  var newString = this.toLowerCase().split(' ');
  // loop through and take the first char in the split words and Uppercase it. then concat the rest of the str.
  for(var i = 0; i < newString.length; i++) {
    newString[i] = newString[i].charAt(0).toUpperCase() + newString[i].substring(1); 
  }
  newStr = newString.join(' '); 
  return newStr;
}
